...
def evaluate_compliance(event, configuration_item, valid_rule_parameters):
    ...
    try:
        eks_name = configuration_item['configuration']['Name']
        AWS_EKS_CLIENT = get_client('eks', event,region="us-east-1")
        response = AWS_EKS_CLIENT.describe_cluster(name=eks_name)
        status=response['cluster']['logging']['clusterLogging'][0]['enabled']
        if status == False:
